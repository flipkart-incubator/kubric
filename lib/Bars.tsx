...
export class CLAppBar
  extends React.Component<CLBarProps, CLBarState>
  implements CLBarInterface
{
  ...
  public static getDerivedState<K extends keyof CLBarState>(
    ...
    let minHeight =
      (nextProps.safeAreaInsets
        ? (nextProps.safeAreaInsets.top || 0) +
          (nextProps.safeAreaInsets.bottom || 0)
        : 0) + (nextProps.showSeparator ? SEPARATOR_HEIGHT : 0);
    let maxHeight = minHeight;

    for (const behaviour of nextProps.scrollBehaviours) {
      const deducedDimensionBehaviour: {
        minHeight: number;
        maxHeight: number;
      } = { minHeight: 0, maxHeight: behaviour.maxHeight };
      ...
      return (
        <Animated.View
          ...
          style={[
            this.props.style,
            {
              height: this.state.heightAnimatedValue,
  ...
  protected renderItems = (
    ...
    let translateYOffset: Animated.AnimatedInterpolation = new Animated.Value(
      0
    );
...
export class CLBottomBar extends CLAppBar {
  ...
  protected renderItems = (
    ...
    const children: React.ReactElement<any>[] = [];
    let index: number = 0;
    console.log("====render Foote =====")
    for (const scrollBehaviour of this.props.scrollBehaviours) {
      const content = this.props.contentRenderer(index, scrollBehaviourOffset);
      console.log("====render Footeer1 =====")
      const alpha =
        scrollBehaviour.scrollEffect & CLScrollEffect.FADE
          ? scrollBehaviourOffset.interpolate({
              inputRange: [0, 0.5, 0.75],
